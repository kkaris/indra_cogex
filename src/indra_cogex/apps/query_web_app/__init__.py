...
This app is running async queries which are only available for neo4j==5.0.0a1
"""
import logging
...
from .models import NodeJson, DefaultQuery
...
Tup = Tuple[str, str]
TupOfTups = Tuple[Tup, ...]
Record = Union[List[List[Any]], None]


@app.post("/get_genes_in_tissue", response_model=List[NodeJson])
async def genes_in_tissue(query: DefaultQuery):
    """Get genes for a disease."""
    # Define the "work" function the way they do it in the example
    async def graph_db_query(tx: Transaction, tissue: str) -> Record:
        n4j_query = """MATCH (g:BioEntity)-[:expressed_in]->(:BioEntity {id: "%s"})
        RETURN g
        """ % tissue
        res = await tx.run(n4j_query)
        return [rec async for rec in res]

    # Now, with an async session, await the read transaction to finish and
    # the process the result
    async with client.get_db() as session:
        result = await session.read_transaction(graph_db_query, query.normalize())
        return [client.neo4j_to_node(res[0]).to_json() for res in result]

    # _, ns, id_ = query.grounding()
    # genes = get_genes_in_tissue(client, (ns, id_))  # <-- async this
    # return [g.to_json() for g in genes]
